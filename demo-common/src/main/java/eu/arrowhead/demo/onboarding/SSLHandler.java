...
public class SSLHandler {
  ...
  public void adaptSSLContext(final String name,
                              final String certificateType,
                              final CertificateResponseDTO response,
                              final String cloudCert,
    ...
    final PrivateKey privateKey = SSLUtilities.parsePrivateKey(response.getPrivateKey(), response.getKeyAlgorithm());
    ...
    storeKeyEntry(String.format("arrowhead-%s-onboarding-certificate", name), privateKey, chain);
  ...
  private Certificate parseCertificate(final String certificateString, final String format)
    ...
    logger.debug("Creating CertificateFactory of type {}", format);
  ...
  private void storeCertificateEntry(String alias, Certificate certificate)
    ...
    if (certificate instanceof X509Certificate) {
      logger.debug("Saving trusted certificate '{}' as '{}'", ((X509Certificate) certificate).getSubjectX500Principal(),
                   alias);
    } else {
